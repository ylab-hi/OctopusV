...
class SVMerger:
    ...
    def get_events_by_source(self, sources, operation='union'):
        ...
        if operation == 'union':
            ...
            other_filtered = [event for event in other_events if
                            any(source in event.source_file.split(',') for source in sources)]
        elif operation == 'intersection':
            tra_filtered = [event for event in tra_events if all(source in event.source_file for source in sources)]
            other_filtered = [event for event in other_events if
                            all(source in event.source_file.split(',') for source in sources)]
